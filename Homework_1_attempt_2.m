...
function output = gmres_helper(v,X,Y,xib,yib,delta,Lap)
    ...
    q = v(end-lq+1:end);
    u = v(1:end-lq);
    Q = spreadQ(X, Y, xib, yib, q, delta);
    1
    N = length(X)
    Phi = reshape(Lap\Q(:),N,N);
    Phi_ib = interpPhi(X,Y,xib,yib,Phi,delta);
    [Phi_ib]
...
function output = gmres_helper(v,X,Y,xib,yib,delta,Lap)
    ...
    size(Phi)
    Phi_ib = interpPhi(X,Y,xib,yib,Phi,delta);
    Phi_ib = reshape(Phi_ib',[],1);
    size(Phi_ib)
    %J = dx * dy * ones(lq, numel(X));
    output = [r_1-r_2;Phi_ib];
