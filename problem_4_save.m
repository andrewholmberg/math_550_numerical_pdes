...
return_points_indices(1,0,4,0,4,[2],[2],.4)
...
slow_log = [];
%215, 
N = [200,215,250,285,315,360,415,500,575,650,750];
N = [310]
for n = N
    n
    [solution,time,error]= run_problem(n,false);
    ['slow ', num2str(time)]
    slow_log = [slow_log;[n,error,time]];
    [solution,time,error]= run_problem(n,true);
    ['fast ', num2str(time)]
    fast_log = [fast_log;[n,error,time]];

end

function [solution,time,p2_error] = run_problem(N,fast)
...
indices = return_points_indices(h,min(Xdom_vect),max(Xdom_vect),min(Ydom_vect),max(Ydom_vect),xib,yib,.01);

if fast
    helper = @(x)gmres_helper_fast(x,N,N_ib,Xdom_vect,Ydom_vect,xib,yib,delta,D_lap,h,indices);
...
time = toc;
u_approx = solution(end-m+1:end);
sum(fmat_ib)-sum(u_approx)

p2_error = sum((fmat_ib' - u_approx).^2)/sum(fmat_ib.^2);
...
%% Functions
% mat to solve system ***
function [Sq] = spreadQ_fast(X,Y,xib,yib,N_ib,q,delta,indices)
    Sq = 0*X;
    % Nq = length(q);
    for k = 1:N_ib
...
function [Jphi] = interpPhi_fast(X,Y,xib,yib,N_ib,Phi,delta,h,indices)
    Jphi = 0*xib;
    dx = h;
    dy = h;
...
function A = gmres_helper_fast(x,N,N_ib,X,Y,xib,yib,delta,D_lap,h,indices)
    vec_len = (N-2)^2 + N_ib;
    u_len = (N-2)^2;

    % apply functions to u
    Ax(1:u_len) = D_lap*x(1:u_len);
    Ax(u_len+1:vec_len) = interpPhi_fast(X,Y,xib,yib,N_ib,x(1:u_len),delta,h,indices); %J(x(1:N-2)),  J(u)

    % apply functions to q
    % test = spreadQ(X,Y,xib,yib,N_ib,x(u_len+1:vec_len),delta) ;
    Ax(1:u_len) =  Ax(1:u_len) - (spreadQ_fast(X,Y,xib,yib,N_ib,x(u_len+1:vec_len),delta,indices))' ; %Ax(1:N-2) - S(x(N-1:vec_len)); -S(q)
...
function m =  return_points_indices(h,xmin,xmax,ymin,ymax,xib,yib,tol)
    reach = abs(norminv(tol/2,0,1)) * 1.2*h;
    reach;
    ...
    for i = 1:length(xib)
        ...
        indices = [];
        for x = xmind : xmaxd
            indices = [indices; (ymind+1 + x * nypoints):1:(ymaxd+1 + x * nypoints)];
        end
